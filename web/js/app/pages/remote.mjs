...
export function Remote({ active, socket, blinds, groups, addSystemTimeListener }) {
  ...
  const runRemoteControll = async (id, action) => {
    clearTimeout(actionIconTimeout);
    
    let response = await socket.send('remote:action', { target: target, targetId: currentTarget.id, action: action });
                  ...
                  <div class="fw-bold text-secondary-emphasis">
                    <div class="row px-2">
                      ...
                      <div class="col text-end">
                        ${actionError && html`
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-x-circle-fill text-danger blink" viewBox="0 0 16 16">
