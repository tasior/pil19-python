...
const blinds = [
  { channel: 0, name: 'Wszystkie' },
  { channel: 1, name: 'Salon' },
  { channel: 2, name: 'Taras' },
];
const definedBlinds = blinds.map(b => b.channel);
...
export function Blinds({ active, socket }) {
    ...
    const onSubmit = e => {
      try {
        const response = socket.send(`blind:${action}`, blind);
          ...
          <div class=" overflow-auto h-100">
            <div class="row p-0 m-0">
              ...
              <div class="col col-12 py-0 text-start">
                ${blinds.map(blind => html`
                  <div class="row gy-2 p-0 m-0">
                    <div class="col col-6 bg-secondary-subtle py-2">
                      <div class="p-2"><strong>${('0' + blind.channel).slice(-2)}</strong> ${blind.name}</div>
                    </div>
                    <div class="col col-6 bg-secondary-subtle py-2 text-end">
                      ...
                      <button class="btn p-2" onClick=${_ => showModal('edit', blind)}>
                      ...
                      <button class="btn p-2 text-danger" onClick=${_ => showModal('remove', blind)}>
              ...
              <div class="col col-12 py-0 text-start">
                <div class="row gy-2 p-0 m-0">
                  <div class="col col-12" style="--bs-bg-opacity: .5;">
                    <button class="btn p-2" onClick=${_ => showModal('add', { channel: '', name: '' })}>
                  ...
                  <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        ${error ? html`<div class="form-floating pb-3"><span class="text-left text-danger error-message">${error}<br/></span></div>` : ''}
                        <div class="form-floating pb-2">
                          ...
                          >
                            ${[...Array(100).keys()].map( i => html`
                              <option 
                                value="${i}" 
                                disabled=${definedBlinds.indexOf(i) > -1}
                              >
                                ${('0' + i).slice(-2)}
                              </option>
                            `)}
